...
const imageSelected = async (selectedImage) => {

    const apiClient = new APIClient(`products/${id}`);

    const res = await apiClient.patch({thumbnail: `common/images/${encodeURIComponent(userStore.user.firstName)}/${selectedImage.name}`});
            ...
            <div class="relative w-full h-full max-w-screen-md mx-auto grid items-center p-2">
                <div class="flex flex-col">
                    ...
                    <ImagesList mode="select">
                        <template v-slot="{selectedImage}">
                            <SolidBtn label="select" size="small" @click="imageSelected(selectedImage)" />
                        </template>
                    </ImagesList>
                </div>
            </div>
